...
class FeedFowardNet(nn.Module):
    def __init__(self, flattened_input_size, flattened_hidden_size, flattened_output_size):
        super(FeedFowardNet, self).__init__()
        self.fc1 = nn.Linear(flattened_input_size, int(flattened_hidden_size))
        self.fc2 = nn.Linear(int(flattened_hidden_size), int(flattened_hidden_size/2))
        self.fc3 = nn.Linear(int(flattened_hidden_size/2), int(flattened_hidden_size/4))
        self.out = nn.Linear(int(flattened_hidden_size/4), flattened_output_size)
    def forward(self, x):
...
def TrainFeedForwardModel(x_train,y_train, param = "T_CALC"):
    x_train = torch.FloatTensor(x_train)
    y_train = torch.FloatTensor(y_train)
    # Create DataLoader
    train_dataset = TensorDataset(x_train, y_train)
    ...
    # Initialize the model
    model = FeedFowardNet(20*56*48, 512, 20*56)
    ...
    for epoch in range(50):
        ...
        for inputs, targets in train_loader:
            # Forward pass
            outputs = model(inputs.view(batch_size,48*56*20))
            loss = criterion(outputs, targets.view(batch_size,56*20))
    ...
    print('Training finished!')
    torch.save(model, "models/FeedFowardNet.pt")
    return model
...
def MakePrediction(start_date, dates, hours, model, this_df, display = True):
    ...
    for i in range(hours):
        x = torch.FloatTensor(input_vector).view(56*48*20)
        raw_output = model(x)
        input_vector.pop(0)
    ...
    file_path = "models/animations/predictions/feed_forward_"+date_time+".pt"
...
def AnimateModel(start_date, dates, hours, model, this_df, truths = True):
    start_index = np.where(dates==start_date)[0][0]
    input_vector = []
    model.eval()
    ...
    plt.show()
    anim.save('animation.mp4', fps=6, writer='ffmpeg')
...
def update(frame):
    ...
    for i in np.linspace(1,240,60):
        valid_hours.append(int(np.floor(i)))
    ax.set_title("Ground Truth, Hour "+str(valid_hours[int(frame)]))
    # Add map features
    ax.coastlines()
    ax.gridlines()
    # Set plot extent to the United States
...
if train_net:
    model = TrainFeedForwardModel(x_train,y_train)
else:
    model = torch.load("models/FeedFowardNet.pt")
