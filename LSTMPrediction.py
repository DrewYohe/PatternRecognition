...
class LSTMModel(nn.Module):
    def __init__(self, input_size = 20*56, hidden_size = 20*56, num_layers = 3, output_size = 20*56):
...
def TrainLSTMModel(x_train,y_train, param = "T_CALC"):
    x_train = torch.FloatTensor(x_train)
    y_train = torch.FloatTensor(y_train)
    # Create DataLoader
    train_dataset = TensorDataset(x_train, y_train)
    ...
    for epoch in range(80):
        ...
        for inputs, targets in train_loader:
            # Forward pass
            outputs = model(inputs.view(20,48,56*20))
        ...
        epoch_loss = running_loss / len(train_dataset)
        print(f'Epoch [{epoch+1}/{80}], Loss: {epoch_loss:.4f}')

    print('Training finished!')
    torch.save(model, "models/LSTMNet.pt")
    return model
...
def MakePrediction(start_date, dates, hours, model, this_df, display = True):
    ...
    file_path = "models/animations/predictions/LSTM_"+date_time+".pt"
...
def AnimateModel(start_date, dates, hours, model, this_df, truths = False):
    ...
    plt.show()
    anim.save('animation_LSTM.mp4', fps=6, writer='ffmpeg')
...
def update(frame):
    ...
    for i in np.linspace(1,240,60):
        valid_hours.append(int(np.floor(i)))
    ax.set_title("LSTM Prediction, Hour "+str(valid_hours[int(frame)]))
    # Add map features
    ax.coastlines()
    ax.gridlines()
    # Set plot extent to the United States
...
if train_net:
    model = TrainLSTMModel(x_train,y_train)
else:
    model = torch.load("models/LSTMNet.pt")
